...
The  goal project is to replace the gene names with Entrez ID extracted from the “gene_info” file

## Reading Homo_sapiens.gene_info file

```{R}
# reading the important  R Library
library(tidyverse)
library(stringr)
library(readr)
...
The file contains a lot of information, but following columns are important: 2, 3, 5. These are GeneID (Entrez Id), Symbol and Synonyms.
```{R}
...
### Exploded the Synonyms features


The Synonym column have multiple Symbol names separated by "|" (pipe), like this AACT|ACT|GIG24|GIG25. extracted the individual gene names from this column add them to the Symbol to GeneId map.
...
## Reading symbols.gmt file

"h.all.v2023.1.Hs.symbols.gmt" This is a gene matrix transposed file. This is a tab-delimited text file. This type of file is used for pathway analysis. It is a very simple file format, however, each line can be of different lengths. This file, therefore, should be read line by line, not by columns. If split each line on tab, it will create a number of values (or fields). The first two values are “pathway name" and”pathway description". All subsequent values are gene names that belong to the particular pathway. 
...
The goal is to replace the gene names with Entrez ID extracted from the first “gene_info” file
```{R}
